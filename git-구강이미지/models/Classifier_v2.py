...
class ResidualSEBlockFC(nn.Module):
    def __init__(self, input_channels=512, hidden_dims=[1024, 512], output_dim=256, dropout_prob=0.4):
        super(ResidualSEBlockFC, self).__init__()
        
        # SE
        self.se_block = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),  # GAP
            nn.Conv2d(input_channels, input_channels // 8, kernel_size=1),
            nn.ReLU(),
            nn.Conv2d(input_channels // 8, input_channels // 8, kernel_size=1),
            nn.ReLU(),  
            nn.Conv2d(input_channels // 8, input_channels, kernel_size=1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        #Res 256
        res1 = self.residual_connection1(x)
        ...
        #Res 256
        res2 = self.residual_connection2(x)
